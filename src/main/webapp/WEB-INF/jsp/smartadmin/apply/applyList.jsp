				...
				<div class="form-group">
					...
					<div class="col-sm-3">
					 ...
					 <!-- <button class="btn btn-primary " data-toggle="modal" data-target="#myModal" onclick="addApply()">报销申请</button> -->
						<!-- &nbsp;&nbsp;&nbsp;&nbsp;
						&nbsp;&nbsp;&nbsp;&nbsp; -->
					  <!-- <a class="btn btn-primary" onclick="downExcel()">
		              <i class="fa fa-plus"></i> 下载  -->
		              <input type="button" value="下载" onclick="downExcel()" class="btn btn-primary">
					</div>
				</div>
			</form>
...
var downExcel=function(){
	$.post("apply/downLoadExcel.do",{"aProposer":'${userId}',"aState":'${aState}'},function(data){
		/*  console.log("成功");
		console.log("data:"+data.data);*/
		data = encodeURI(encodeURI(data.fileName)); 
		window.location.href="<%=basePath%>/filedownload/queryFileDownload.do?tableName='"+data+"'";
	});
} 
	...
	function run_jqgrid_function() {
		...
		jQuery("#jqgrid").jqGrid({
			...
			colNames : ['操作','报销序号','报销名称','开票单位','发票编号','报销金额','发票类型','发票用途','项目名称','申请人','审批人','审批结果','说明',
			            '开票时间','申报时间','申请人ID','审批人ID','项目ID','类型ID','状态ID','总金额'],		
			colModel : [
						...
						{ name : 'totalMoney', index : 'totalMoney', hidden : false},
					  ], 	
			rowNum : 10,
			rowList : [10, 20, 30],
			pager : '#pjqgrid',
			...
			gridComplete: function(){
				...
				for(var i=0;i < ids.length;i++){
					...
					be = "<button class='btn btn-xs btn-default' title='修改' onclick=\"editRow('"+id+"','"+stateInt+"');\"><i class='fa fa-pencil'></i></button>"; 
					...
					jQuery("#jqgrid").jqGrid('setRowData',ids[i],{act:be+se});
				}	
			},
			caption : "我的报销单",				
			autowidth : true,
